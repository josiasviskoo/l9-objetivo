			...
			<div class="footer-brand">
				...
				<div class="socials">
					...
					foreach ( $socials as $key => $social ) :
						list( $label, $icon_svg ) = $social;
						$url = objetivo_opt( 'footer', $key );
						if ( ! $url ) {
							continue;
						}
						?>
						<a href="<?php echo esc_url( $url ); ?>" target="_blank" rel="noopener noreferrer" title="<?php echo esc_attr( $label ); ?>" aria-label="<?php echo esc_attr( $label ); ?>"><?php echo $icon_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- SVG markup fixo definido acima, mesmo padrão dos ícones de telefone/WhatsApp em header.php. ?></a>
					<?php endforeach; ?>
				</div>
			</div>
